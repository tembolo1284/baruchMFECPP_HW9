...
and then backs out the Put or Call price (the other type) using put-call parity.  Finally, I then check that Put + Call = S - Ke^-rt and 
spit out True if that's the case.
...
functionality in the most flexible/robust way you can think of.
	...
	I created the matrix by creating a vector of vectors and iterating through each row by going column by column first.
...
and price a put option for the last column.  I made this a function that requires certain params to be able
...
Just 5 scenarios would require 30 manual inputs.  I could in theory make a mesh class so I could make a quick mesh on demand
for any param if users found that useful.

	One coding point I noticed that I think would help if it got cleaned up was the toggle() function's use.  Stripping that out
of main and just doing it purely behind the scenes would be cleaner. Since I am doing toggle() in the main function, I couldn't
make the option object const when feeding it in as an input.
...
delta and gammas instead of prices in the loop.
...
My output is : check C = 18.5035, P = 3.03106
Call Px: 18.5
Put Px: 3.031
...
Code is 99% identical to what I had above.
...
Just like in c) above, the code is 99% identical. I just created a new function and then print the matrix
